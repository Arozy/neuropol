function runFillers() {
    const staff = new Staff('nasi-specjalisci');
    staff.fillHeaderTemplate();
    staff.fillSpecialistsTemplate();
    staff.fillFooterTemplate();
}

class Staff extends Page {
    fillSpecialistsTemplate() {
        const noImage = 'data:image/png;base64,';
        const data = {
            header: 'Nasi specjaliści',
            description: '',
            staff: [
                {
                    title: 'Fizjoterapia',
                    description: '',
                    singleItemTitle: 'Fizjoterapeuta',
                    items: [
                        {
                            image: noImage,
                            alt: 'Uśmiechnięta osoba patrzy w stronę aparatu',
                            name: 'Mgr Karol Kuśmirek',
                            description: ''
                        },
                    ]
                },
                {
                    title: 'Neurologopedia',
                    description: '',
                    singleItemTitle: 'Neurologopeda',
                    items: [
                        {
                            image: noImage,
                            alt: 'Uśmiechnięta osoba patrzy w stronę aparatu',
                            name: 'Mgr Agnieszka Krause',
                            description: ''
                        },
                    ]
                },
                {
                    title: 'Terapia zajęciowa',
                    description: '',
                    singleItemTitle: 'Terapeuta',
                    items: [
                        {
                            image: noImage,
                            alt: 'Uśmiechnięta osoba patrzy w stronę aparatu',
                            name: 'Mgr Monika Piotrowska',
                            description: ''
                        }
                    ]
                }
            ]
        }
        this.useFiller(data, 'specialists');
    }
}
