function runFillers() {
    const staff = new Staff('nasi-specjalisci', 'pages/3', '?populate=deep');
    staff.build().finally();
}

Handlebars.registerHelper('placeholder', function (context) {
    if (!context) {
        return 'data:image/png;base64,';
    } else {
        return context;
    }
});

class Staff extends Page {
    async fillSpecialistsTemplate() {
        if (await this.fetchAndLoad()) {
            const data = {
                ...this.response,
                items: await this.getExact('staff.staff')
            }

            this.useFiller(data, 'specialists');
        }
    }

    async build() {
        const allMethods = [
            this.fillHeaderTemplate(),
            this.fillSpecialistsTemplate(),
            this.fillFooterTemplate()
        ];

        for (const method of allMethods) {
            await method;
        }

        document.querySelector('.preloader').classList.add('preloader-deactivate');
    }
}
